...
import { Edit, Delete, FilterList, AddCircle } from "@material-ui/icons";
  ...
  const handleChange = event => {
    ...
    switch (event.target.name) {
      ...
      case "manufacturer-filter":
        ...
        filters["manufacturerFilter"] = event.target.value;
        break;
      case "symbol-filter":
        setSymbolFilter(event.target.value);
        filters["symbolFilter"] = event.target.value;
        break;
      default:
        console.log("error");
    }
    ...
    models = models.filter(row => {
      return row["name"].toLowerCase().startsWith(filters["nameFilter"]);
    });
    models = models.filter(row => {
      return row["manufacturer"]
        .toLowerCase()
        .startsWith(filters["manufacturerFilter"]);
    });
    models = models.filter(row => {
      return row["symbol"].toLowerCase().startsWith(filters["symbolFilter"]);
    });
    ...
    <div id="airplane-models-table-container">
      <Paper>
        <Toolbar>
          ...
          <Link to="/airplane-models/add-airplane-model">
            <IconButton edge="end">
              <AddCircle />
            </IconButton>
          </Link>
